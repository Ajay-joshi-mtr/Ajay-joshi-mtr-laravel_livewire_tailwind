<form method="POST" wire:submit.prevent="submit" x-show="show" x-transition:enter="fade">
    ...
    <label class="mt-4">Alternate Mobile : </label>

    <x-inputs.mobile key="mobile_alt" required="required" autofocus />
  	@error('mobile_alt') <p class="text-red-600 text-xs italic">{{ $message }} </p> @enderror
