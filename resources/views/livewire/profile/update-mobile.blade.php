<form method="POST" wire:submit.prevent="submit" x-show="show" x-transition:enter="fade">
    ...
    <label class="mt-4">New Mobile : </label>

    <x-inputs.mobile key="mobile" required="required" autofocus />
  	@error('mobile') <p class="text-red-600 text-xs italic">{{ $message }} </p> @enderror
